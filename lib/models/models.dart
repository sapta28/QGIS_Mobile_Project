class BillboardModel {
  ...
  factory BillboardModel.fromJson(Map<String, dynamic> json) {
    return BillboardModel(
      ...
      type: json['category'] is Map ? (json['category']['name'] ?? 'Billboard') : (json['category'] ?? json['type'] ?? 'Billboard'),
      pricePerWeek: double.tryParse(json['active_pricing']?['price_per_month']?.toString() ?? json['pricePerWeek']?.toString() ?? '0') ?? 0,
      isHeldByOthers: json['is_held_by_others'] ?? json['isHeldByOthers'] ?? false,
      size: json['size'] ?? '14\' × 48\'',
      traffic: json['traffic_density'] ?? json['traffic'] ?? 'Medium',
      dailyImpressions: json['impressions_per_day'] ?? json['dailyImpressions'] ?? 0,
      isAvailable: json['is_active'] ?? json['isAvailable'] ?? false,
      description: json['description'] ?? '',
      direction: json['facing_direction'] ?? json['direction'] ?? '',
      ...
      printFee: double.tryParse(json['print_fee']?.toString() ?? json['printFee']?.toString() ?? '1500000'),
      installFee: double.tryParse(json['install_fee']?.toString() ?? json['installFee']?.toString() ?? '500000'),
      taxRate: double.tryParse(json['tax_rate']?.toString() ?? json['taxRate']?.toString() ?? '0.11'),
      downPaymentRate: double.tryParse(json['down_payment_rate']?.toString() ?? json['downPaymentRate']?.toString() ?? '0.30'),
    );
  }
